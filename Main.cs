...
namespace DataAnalysisTool
{
    ...
    (e.KeyChar != '.'))
        ...
        private void button27_Click(object sender, EventArgs e)
        {
            ...
            conn.Open();
            SqlCommand sc = new SqlCommand("use " + databaseSelect4.Text + " select importformatid as name from ImportFormat", conn);

            //for version 7.0
            var selectCodeType1 = "USE " + databaseSelect4.Text + " SELECT ef.codetype FROM ImportFormat IMF INNER JOIN ImportFormatEntity IMFE ON IMF.ImportFormatNo= IMFE.ImportFormatNo INNER JOIN ImportFormatField IMFF ON IMF.ImportFormatNo = IMFF.ImportFormatNo  left JOIN EntityField EF ON ef.entname=imfe.inentname and ef.fldname=IMFF.ImportFormatFieldId where imf.importformatid = " + @"'" + ifSelect.Text + @"'" + "  and IMF.QBQueryNo is null and ef.valuetype=1 order by imff.FieldSeq";
            //for version 2018
            var selectCodeType2 = "USE " + databaseSelect4.Text + " SELECT ct.codetypeid FROM ImportFormat IMF INNER JOIN ImportFormatEntity IMFE ON IMF.ImportFormatNo= IMFE.ImportFormatNo INNER JOIN ImportFormatField IMFF ON IMF.ImportFormatNo = IMFF.ImportFormatNo  left JOIN EntityField EF ON ef.entname=imfe.inentname and ef.fldname=IMFF.ImportFormatFieldId left join codetype ct on ef.codetypeno=ct.codetypeno where imf.importformatid = " + @"'" + ifSelect.Text + @"'" + "  and IMF.QBQueryNo is null and ef.valuetype=1 order by imff.FieldSeq";
            var runListNoRoot = "USE " + databaseSelect4.Text + " select distinct rl.runlistnoroot from RunList rl left join rundet rd on rd.runlistno = rl.runlistno where rd.ItemName = 'PayoutTypeNo' and rd.ItemValue = (select payouttypeno from PayoutType where payouttypeid = '"+payoutTypeSelect.Text+"') and rl.DatFrom = '"+payoutSelect.Text+"'";
            var dataAdapter1 = new SqlDataAdapter(selectCodeType1, conn);
            var dataAdapter2 = new SqlDataAdapter(runListNoRoot, conn);
